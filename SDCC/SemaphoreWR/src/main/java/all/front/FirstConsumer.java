...
//            "localhost:9092,localhost:9093,localhost:9094";
    ...
    public void setController(SemaphoreController semaphoreController){
        createConsumer();
...
//        System.out.println("DONE");
    ...
    private void workWithMessage(Message message){
        ...
        if (code == 1){
            ...
            list.add(new Crossroad(message.getID(), ""));
            x.setCrossroads(list);
            semaphoreController.addToSemaphoreList(x);
        }
        else if (code == -1){
            semaphoreController.removeFromSemaphoreList(message.getSemaphore());
        }
        else if (code == 10){
            semaphoreController.setSemaphoreList(message.getListOfSemaphores(), message.getID());
        }
        else if (code == 401) {
            semaphoreController.sendStatus(message.getCurrentCycle());
        }
    }
